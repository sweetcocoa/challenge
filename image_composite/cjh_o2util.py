...
def project(mesh, fx, fy, cx, cy):
    dest = np.zeros((mesh.shape[0], 2))

    # devide by zero 방지
    mesh[:,2] = mesh[:,2] + 1e-12

    dest[:,0] = (mesh[:,0] * fx / mesh[:,2]) + cx
    dest[:,1] = (mesh[:,1] * fy / mesh[:,2]) + cy
    # print('calc x:', (mesh[0,0] * fx / mesh[0,2]) + cx, ', ', dest[0,0])
    # print('calc y:', (mesh[0,1] * fy / mesh[0,2]) + cy, ', ', dest[0,1])
    return dest


def project_mean_3d(fov, sz, t, r, y_shrink) :
    mean_3d = get_mean_3d(y_shrink)
    mean_3d = mean_3d.reshape(68, 3)
    world_3d = obj2world_list(mean_3d, r, t)

    fx, fy, cx, cy = fov_to_camera_parameter(fov, sz[0], sz[1])
    points_2d = project(world_3d, fx, fy, cx, cy)
    return points_2d

# o2 용 profile 계산해주는 함수
# input : landmark
def o2_profile(fov, landmark_2d, sz, y_shrink, image=None, key=None):
    ...
    #     path = "/home/snowyunee/data/est/deep_fit/" + key + ".png"
    #     print("path:", path)
    #     save_plt(image, landmark_2d, path)
